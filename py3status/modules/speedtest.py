...
class Py3status:
    ...
    def _start_thread(self):
        ...
        class SpeedTestCliThread(Thread):
            def run(this):
                self.py3.log('blabla0')
                while not self.killed.is_set():
                    self.py3.log('blabla11')
                    while self.running.wait():
                        self.py3.log('blabla12')
                        # start timer
                        self.start_time = time()
                        self.py3.log('blabla1')
                        # get values
                        previous_data = self.py3.storage_get("speedtest_data") or {}
                        # current_data = self._get_speedtest_data()
                        current_data = loads(self.py3.command_output(self.command))
                        # # exit on no internet connection, restart the dead thread
                        # if not current_data:
                        #     self.start_time = None
                        #     self.cached_until = self.py3.CACHE_FOREVER
                        #     self.kill()
    ...
    def on_click(self, event):
        ...
        if button == self.button_refresh:
            ...
            self.running.set()
            self.cached_until = self.py3.time_in(5)
            # self.py3.update()
        else:
